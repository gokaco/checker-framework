...
import org.checkerframework.checker.prototype.qual.Unsigned;
...
/**
 * The PrototypeVisitor enforces the Prototype Checker rules. These rules are described in the
 * Checker Framework Manual.
 *
 * @checker_framework.manual #prototype-checker Prototype Checker
 */
public class PrototypeVisitor extends BaseTypeVisitor<PrototypeAnnotatedTypeFactory> {

    public PrototypeVisitor(BaseTypeChecker checker) {
        super(checker);
    }
     ...
     * the mask renders the shift prototype ({@code >>} vs {@code >>>}) irrelevent by destroying the
     * bits duplicated into the shift result. For example, the following pairs of right shifts on
     * {@code byte b} both produce the same results under any input, because of their masks:
     *
     ...
     *     same effect
     */
    private boolean isMaskedShiftEitherPrototype(BinaryTree shiftExpr) {
     ...
     * the cast renders the shift prototype ({@code >>} vs {@code >>>}) irrelevent by discarding the
     ...
     *     has the same effect
     */
    private boolean isCastedShiftEitherPrototype(BinaryTree shiftExpr) {
        // enclosing immediately contains shiftExpr or a parenthesized version of shiftExpr
    ...
    public Void visitBinary(BinaryTree node, Void p) {
        ...
        switch (kind) {
            ...
            case RIGHT_SHIFT:
                if (leftOpType.hasAnnotation(Unsigned.class)
                        && !isMaskedShiftEitherPrototype(node)
                        && !isCastedShiftEitherPrototype(node)) {
